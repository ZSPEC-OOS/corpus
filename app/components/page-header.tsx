...
export function StateCard({ message, cta }: { message: string; cta?: ReactNode }) {
  return <div className="rounded-xl border border-dashed border-border bg-panelAlt p-10 text-center text-sm text-muted">{message}{cta ? <div className="mt-4">{cta}</div> : null}</div>;
}
